<template>
  <div class="menu-page container pb-2">
      ...
      <div class="col text-center">
        ...
        <p class="mt-2 mb-2">
          <small
            ...
            <help-tooltip
              v-if="game.settings.general.flux.tooltip"
              :tooltip="game.settings.general.flux.tooltip"
          /></small>
        </p>
      </div>
    </div>
    ...
    <div class="row bg-dark" v-if="!game.state.endDate">
      <div class="col text-center pt-2">
        <p class="mb-2">
          Galactic Cycle {{ $store.state.productionTick }} - Tick
          {{ $store.state.tick }}
        </p>
        <p
          class="text-warning"
          v-if="isDarkModeExtra && getUserPlayer() != null"
        >
    ...
    <div class="row bg-success" v-if="game.state.endDate">
      <div class="col text-center pt-2">
        ...
        <p>
          The winner is
          {{ game.settings.general.teamGame === "enabled" ? "Team: " : ""
          }}<b>{{ getWinnerAlias() }}</b
          >!
        </p>
      </div>
            ...
            <tr v-for="player in sortedPlayers" :key="player._id">
              ...
              <td class="ps-2 pt-3 pb-2">
                ...
                <h5 class="alias-title">
                  {{ player.alias }}
                  {{
                    game.settings.general.teamGame === "enabled"
                      ? ` - ${player.teamName}`
                      : ""
                  }}
                  <span
              ...
              <td
                class="fit pt-3 pe-2"
                v-if="isConquestAllStars || isKingOfTheHillMode"
              >
              ...
              <td
                class="fit pt-2 pb-2 pe-1 text-center"
                v-if="isTurnBasedGame && canEndTurn"
              >
                <h5
                  v-if="player.ready && !isUserPlayer(player)"
                  class="pt-2 pe-2 ps-2"
                >
                  <i
      ...
      <div class="col text-end pe-2">
        ...
        <button
          v-if="
            canReadyToQuit &&
              !getUserPlayer().defeated &&
              !getUserPlayer().readyToQuit
          "
          @click="confirmReadyToQuit(getUserPlayer())"
        ...
        <button
          v-if="
            canReadyToQuit &&
              ...
              getUserPlayer().readyToQuit
          "
          @click="unconfirmReadyToQuit(getUserPlayer())"
...
export default {
  ...
  mounted() {
    ...
    if (this.$store.state.game.settings.general.teamGame === "enabled") {
      // lets loop through the players and get their team name:
      this.players.forEach(player => {
        let teams = this.$store.state.game.galaxy.teams;

        let playerTeam = teams.find(team => {
          return team.members.some(
            member => member.toString() === player._id.toString()
          );
        });
        console.log(playerTeam.teamName);
        player.teamName = playerTeam.teamName;
      });
    }
    ...
    if (
      GameHelper.isGameInProgress(this.$store.state.game) ||
      GameHelper.isGamePendingStart(this.$store.state.game)
    ) {
  ...
  methods: {
    ...
    recalculateTimeRemaining() {
      if (gameHelper.isRealTimeGame(this.$store.state.game)) {
        this.timeRemaining = `Next tick: ${gameHelper.getCountdownTimeStringByTicks(
          this.$store.state.game,
          1
        )}`;
      } else if (gameHelper.isTurnBasedGame(this.$store.state.game)) {
        this.timeRemaining = `Next turn: ${gameHelper.getCountdownTimeStringForTurnTimeout(
          this.$store.state.game
        )}`;
      }
    },
    async quitGame() {
      ...
      try {
        ...
        if (response.status === 200) {
          AudioService.quit();
          this.$toasted.show(
            `You have quit ${this.$store.state.game.settings.general.name}.`,
            { type: "error" }
          );
    ...
    async confirmReady(player) {
      ...
      try {
        let response = await gameService.confirmReady(
    ...
    async confirmReadyToCycle(player) {
      ...
      try {
        let response = await gameService.confirmReadyToCycle(
          this.$store.state.game._id
        );
    ...
    async unconfirmReady(player) {
      ...
      try {
        let response = await gameService.unconfirmReady(
          this.$store.state.game._id
        );
    ...
    async confirmReadyToQuit(player) {
      ...
      if (
        !(await this.$confirm(
          "Ready to Quit?",
          "Are you sure you want declare that you are ready to quit? If all active players declare ready to quit then the game will end early."
        ))
      ) {
        return;
      }
      ...
      try {
        let response = await gameService.confirmReadyToQuit(
          this.$store.state.game._id
        );

        if (response.status === 200) {
          this.$toasted.show(`You have confirmed that you are ready to quit.`, {
            type: "success"
          });
    ...
    async unconfirmReadyToQuit(player) {
      ...
      try {
        let response = await gameService.unconfirmReadyToQuit(
          this.$store.state.game._id
        );
    ...
    getWinnerAlias() {
      let winnerPlayer = GameHelper.getPlayerById(
        this.$store.state.game,
        this.$store.state.game.state.winner
      );

      // TODO use a service to keep it dry
      if (this.$store.state.game.settings.general.teamGame === "enabled") {
        // get team Name and return that as the "winning" alias
        const teams = game.galaxy.teams;

        // Find the team that the player is in
        const playerTeam = teams.find(team => {
          return team.members.some(memberObj => memberObj === member);
        });
  ...
  computed: {
    ...
    canReadyToQuit() {
      return (
        this.$store.state.game.settings.general.readyToQuit === "enabled" &&
