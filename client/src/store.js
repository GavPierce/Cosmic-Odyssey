  ...
  mutations: {
    ...
    setMenuState(state, menuState) {
      ...
      if (
        ...
        menuState.args === state.menuArguments
      ) {
    ...
    closeConversation(state) {
      if (state.currentConversation) {
        ...
        state.cachedConversationComposeMessages[id] =
    ...
    gameStarAllShipsTransferred(state, data) {
      ...
      data.carriers.forEach(carrier => {
        let mapObjectCarrier = GameHelper.getCarrierById(
          state.game,
          carrier._id
        );
    ...
    gameStarAbandoned(state, data) {
      ...
      let carriers = state.game.galaxy.carriers.filter(
        x =>
          x.orbiting &&
          x.orbiting === star._id &&
      ...
      carriers.forEach(c => {
        ...
        state.game.galaxy.carriers.splice(
          state.game.galaxy.carriers.indexOf(c),
          1
        );
      });
    ...
    gameCarrierScuttled(state, data) {
      ...
      state.game.galaxy.carriers.splice(
        state.game.galaxy.carriers.indexOf(carrier),
        1
      );
