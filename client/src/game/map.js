...
class Map extends EventEmitter {
  ...
  constructor(app, store, gameContainer) {
    ...
    this.stars = [];
    this.carriers = [];
    // An armada is 2 or more friendly carriers in the same location.
    this.armadas = {};
  ...
  setup(game, userSettings) {
    ...
    this.waypoints.onWaypointCreatedHandler = this.waypoints.on(
      "onWaypointCreated",
      this.onWaypointCreated.bind(this)
    );
    this.waypoints.onWaypointOutOfRangeHandler = this.waypoints.on(
    ...
    this.rulerPoints.onRulerPointCreatedHandler = this.rulerPoints.on(
      "onRulerPointCreated",
      this.onRulerPointCreated.bind(this)
    );
    this.rulerPoints.onRulerPointsClearedHandler = this.rulerPoints.on(
      "onRulerPointsCleared",
      this.onRulerPointsCleared.bind(this)
    );
    this.rulerPoints.onRulerPointRemovedHandler = this.rulerPoints.on(
      "onRulerPointRemoved",
  ...
  setupCarrier(game, userSettings, carrierData) {
    ...
    if (!carrier) {
      ...
      carrier.on(
        ...
        this.onCarrierRightClicked.bind(this)
      );
  ...
  checkForArmadas(carriers) {
    //._PREEEETT_ sure, like 90%, we want to empty the armadas array every time. So that we only get fresh ships that are sharing the current location
    this.armadas = {};
    // check carriers locations to see if any are in the same place. If they are add them to the armada array
    let carrierWithSharedLoc = [];
    let carrierLocations = new Set();

    // if there are two or more carriers in the same location, add them to the armada array
    for (let i = 0; i < carriers.length; i++) {
      let carrier = carriers[i];
      let location = carrier.data.location;
      let locationString = location.x + "," + location.y;
      if (carrierLocations.has(locationString)) {
        carrierWithSharedLoc.push(carrier);
      } else {
        carrierLocations.add(locationString);
      }
    }
    carrierWithSharedLoc.forEach(carrier => {
      let armada = [];
      let carrierLocation = `${carrier.data.location.x},${carrier.data.location.y}`;
      carriers.forEach(c => {
        if (
          c.data.location.x === carrier.data.location.x &&
          c.data.location.y === carrier.data.location.y
        ) {
          armada.push(c);
        }
      });
      if (!this.armadas[carrierLocation]) {
        this.armadas[carrierLocation] = armada;
      }
    });
  }
  draw() {
  ...
  drawCarriers() {
    for (let i = 0; i < this.carriers.length; i++) {
      let carrier = this.carriers[i];
      // if carrier is in any armada skip drawing it
      let carrierLocation = `${carrier.data.location.x},${carrier.data.location.y}`;
      if (this.armadas[carrierLocation]) {
        continue;
      }
  ...
  drawArmadas() {
    // draw armadas, but only first carrier in armada
    for (let armada in this.armadas) {
      let armadasTotalShips = 0;
      for (let i = 0; i < this.armadas[armada].length; i++) {
        armadasTotalShips += this.armadas[armada][i].data.ships;
      }
      if (this.armadas[armada].length > 1) {
        let originalShips = this.armadas[armada][0].data.ships;
        this.armadas[armada][0].data.ships = armadasTotalShips;
        this.drawCarrier(this.armadas[armada][0], true);
        this.armadas[armada][0].data.ships = originalShips;
      }
    }
  }
  drawCarrier(carrier, isArmada = false) {
    if (isArmada) console.log("drawing armada");
    carrier.draw(isArmada);
  ...
  onTick(deltaTime) {
    let viewportWidth =
      this.gameContainer.viewport.right - this.gameContainer.viewport.left;
    let viewportHeight =
      this.gameContainer.viewport.bottom - this.gameContainer.viewport.top;
    ...
    this.zoomPercent =
      (this.gameContainer.viewport.screenWidth / viewportWidth) * 100;
    ...
    this.pathManager.onTick(
      this.zoomPercent,
      this.gameContainer.viewport,
      zoomChanging
    );
