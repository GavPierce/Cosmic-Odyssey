...
class Star extends EventEmitter {
  ...
  */
  ...
  _getStarCarriers() {
    let carriersAtStar = this.carriers.filter(
  ...
  drawNaturalResourcesRing() {
    ...
    for (let lod = 0; lod < Star.maxLod; lod += 1) {
      ...
      lineWidht *= lod + 1;
      this.graphics_natural_resources_ring[lod].clear();
      this.graphics_natural_resources_ring[lod].lineStyle(
        lineWidht,
        0xffffff,
        0.1
      );
      this.graphics_natural_resources_ring[lod].drawCircle(
        0,
        0,
        ringRadius * 0.75
      );
  ...
  drawColour() {
    ...
    if (Object.keys(TextureService.PLAYER_SYMBOLS).includes(player.shape)) {
      this.graphics_shape_part = new PIXI.Sprite(
        TextureService.PLAYER_SYMBOLS[player.shape][2 + this.data.warpGate]
      );
      this.graphics_shape_full = new PIXI.Sprite(
        TextureService.PLAYER_SYMBOLS[player.shape][0 + this.data.warpGate]
      );
    }
    ...
    if (this.data.warpGate) {
      this.graphics_shape_part.filters = [
        new GlowFilter(10, 10, 5, player.colour.value, 0.1)
      ];
      this.graphics_shape_full.filters = [
        new GlowFilter(5, 10, 5, player.colour.value, 1)
      ];
    }
    this.container.addChild(this.graphics_shape_part);
    this.container.addChild(this.graphics_shape_full);
  ...
  drawName() {
    ...
    if (
      ...
      this.text_name.y =
        (Star.nameSize + Star.shipsSmallSize) / 2.0 - Star.nameSize;
    } else {
  ...
  drawInfrastructure() {
    ...
    if (!this.text_infrastructure) {
      if (this.data.ownedByPlayerId && this.data.infrastructure) {
        ...
        this.text_infrastructure = new PIXI.BitmapText(
  ...
  drawInfrastructureBulkIgnored() {
    ...
    if (!this.text_infrastructureBulkIgnored) {
      let displayInfrastructure = `${
        this.data.ignoreBulkUpgrade.economy ? " " : "E"
      } ${this.data.ignoreBulkUpgrade.industry ? " " : "I"} ${
        this.data.ignoreBulkUpgrade.science ? " " : "S"
      }`;

      let bitmapFont = { fontName: "chakrapetch", fontSize: 8 };
      this.text_infrastructureBulkIgnored = new PIXI.BitmapText(
        displayInfrastructure,
        bitmapFont
      );
  ...
  drawScanningRange() {
    ...
    let radius =
      ((this.data.effectiveTechs.scanning || 1) + 1) * this.lightYearDistance;
  ...
  updateVisibility() {
    ...
    if (this.userSettings.map.naturalResources !== "planets") {
      if (this.graphics_natural_resources_ring[lod]) {
        this.graphics_natural_resources_ring[lod].visible =
    ...
    if (this.text_name)
      this.text_name.visible =
        this.isSelected || this.zoomPercent >= Star.zoomLevelDefinitions.name;
    if (this.container_planets)
      this.container_planets.visible =
        this.data.isInScanningRange &&
        this.zoomPercent >= Star.zoomLevelDefinitions.naturalResources;
    if (this.text_infrastructure)
    ...
    let small_ships =
      this.zoomPercent >= Star.zoomLevelDefinitions.name || this.isSelected;
    let visible_ships = !!(
      this.data.isInScanningRange &&
      (this.isSelected ||
        this.isMouseOver ||
