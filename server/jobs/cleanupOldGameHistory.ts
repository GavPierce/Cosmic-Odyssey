...
export default (container: DependencyContainer) => {
  ...
  return {
    async handler(job, done) {
      let games =
        await container.gameListService.listOldCompletedGamesNotCleaned(months);

      for (let i = 0; i < games.length; i++) {
        let game = games[i];

        // TODO: This is a quick bodge to immortalise this game: https://cosmic-odyssey.io//#/game?id=63bcb3de8b616b3bbe0e084b
