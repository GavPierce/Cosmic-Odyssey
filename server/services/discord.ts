...
export default class DiscordService {
  ...
  async initialize() {
    if (this.config.discord.botToken) {
      // Don't initialize the service if there's no token configured.
  ...
  async updateOAuth(userId, discordUserId, oauth) {
    ...
    if (!isServerMember) {
      throw new ValidationError(
        `You must be a member of the official Cosmic Odyssey discord server to continue. Please join the server and try again.`
      );
    }
  ...
  async sendMessageByUserId(discordUserId: string, messageTemplate: any) {
    ...
    // We need to double check that the user is a member of the Cosmic Odyssey discord server
