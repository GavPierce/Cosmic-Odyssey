...
export default class EmailService {
  TEMPLATES = {
    WELCOME: {
      ...
      subject: "Welcome to Cosmic Odyssey",
    },
    RESET_PASSWORD: {
      fileName: "resetPassword.html",
      subject: "Reset your Cosmic Odyssey password",
    },
    FORGOT_USERNAME: {
      fileName: "forgotUsername.html",
      subject: "Your Cosmic Odyssey username",
    },
    GAME_WELCOME: {
      fileName: "gameWelcome.html",
      subject: "Your Cosmic Odyssey game starts soon!",
    },
    GAME_FINISHED: {
      fileName: "gameFinished.html",
      subject: "Your Cosmic Odyssey game has ended!",
    },
    GAME_CYCLE_SUMMARY: {
    ...
    YOUR_TURN_REMINDER: {
      ...
      subject: "Cosmic Odyssey - It's your turn to play!",
    },
    NEXT_TURN_REMINDER: {
      fileName: "nextTurnReminder.html",
      subject: "Cosmic Odyssey - Turn finished, it's your turn to play!",
    },
    GAME_TIMED_OUT: {
      fileName: "gameTimedOut.html",
      subject: "Cosmic Odyssey - Your game did not start",
    },
    GAME_PLAYER_AFK: {
      fileName: "gamePlayerAfk.html",
      subject: "Cosmic Odyssey - You've gone AFK",
    },
    REVIEW_REMINDER_30_DAYS: {
      fileName: "reviewReminder.html",
      subject: "Cosmic Odyssey - How did we do?",
    },
  };
  ...
  constructor(
    ...
    this.gameJoinService.on(GameJoinServiceEvents.onGameStarted, (args) =>
      this.sendGameStartedEmail(args)
    );
    this.userService.on(UserServiceEvents.onUserCreated, (user) =>
      this.sendWelcomeEmail(user)
    );
    this.playerReadyService.on(
      PlayerReadyServiceEvents.onGamePlayerReady,
      (data) => this.trySendLastPlayerTurnReminder(data.gameId)
    );

    this.gameTickService.on(GameTickServiceEvents.onGameTurnEnded, (args) =>
      this.trySendNextTurnReminder(args.gameId)
    );
    this.gameTickService.on(GameTickServiceEvents.onPlayerAfk, (args) =>
      this.sendGamePlayerAfkEmail(args)
    );
    this.gameTickService.on(GameTickServiceEvents.onGameEnded, (args) =>
      this.sendGameFinishedEmail(args.gameId)
    );
    this.gameTickService.on(GameTickServiceEvents.onGameCycleEnded, (args) =>
      this.sendGameCycleSummaryEmail(args.gameId)
    );
  }
  ...
  _getTransport() {
    ...
    if (this.isEnabled()) {
      var transporter = nodemailer.createTransport({
        service: "Mailgun",
        auth: {
          user: this.config.smtp.mailGunUsername,
          pass: this.config.smtp.mailGunPassword,
        },
      });
      return transporter;
    } else {
  ...
  async sendTemplate(toEmail: string, template: EmailTemplate, parameters) {
    ...
    html = html.replace("[{Cosmic Odyssey_url}]", this.config.clientUrl);
    html = html.replace(
      "[{Cosmic Odyssey_url_gamelist}]",
      `${this.config.clientUrl}/#/game/list`
    );
    html = html.replace(
      "[{Cosmic Odyssey_url_resetpassword}]",
      `${this.config.clientUrl}/#/account/reset-password-external`
    );
    html = html.replace(
      "[{source_code_url}]",
      "https://github.com/Cosmic Odyssey-games/Cosmic Odyssey"
    );
  ...
  async sendGameCycleSummaryEmail(gameId: DBObjectId) {
    ...
    let undefeatedPlayers = game.galaxy.players.filter(
    ...
    switch (game.settings.general.mode) {
      ...
      case "battleRoyale":
        winConditionText =
          'Winner will be the <span style="color:#3498DB;">last man standing</span>.';
        break;
      case "kingOfTheHill":
        winConditionText =
          'Winner will be the player who <span style="color:#3498DB;">captures and holds</span> the center star.';
        break;
    }
    ...
    for (let player of undefeatedPlayers) {
      await this._trySendEmailToPlayer(
        player,
        this.TEMPLATES.GAME_CYCLE_SUMMARY,
        [gameName, gameUrl, winConditionText, leaderboardHtml]
      );
    }
  }
  ...
  async trySendLastPlayerTurnReminder(gameId: DBObjectId) {
    ...
    let undefeatedPlayers = game.galaxy.players.filter(
      (p: Player) => !p.defeated && !p.ready && p.userId
    );
    ...
    if (undefeatedPlayers.length === 1) {
      ...
      await this._trySendEmailToPlayer(
        player,
        this.TEMPLATES.YOUR_TURN_REMINDER,
        [gameName, gameUrl]
      );
    }
  }
  ...
  async trySendNextTurnReminder(gameId: DBObjectId) {
    ...
    if (
      ...
      !this.gameStateService.isInProgress(game) ||
    ...
    for (let player of undefeatedPlayers) {
      await this._trySendEmailToPlayer(
        player,
        this.TEMPLATES.NEXT_TURN_REMINDER,
        [gameName, gameUrl]
      );
    }
  }
