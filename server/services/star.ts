...
export default class StarService extends EventEmitter {
  ...
  listStarsOwnedByPlayers(stars: Star[], playerIds: DBObjectId[]) {
    ...
    return stars.filter(
      (s) => s.ownedByPlayerId && ids.includes(s.ownedByPlayerId.toString())
    );
  }
  ...
  listStarsWithScanningRangeByPlayer(game: Game, playerId: DBObjectId): Star[] {
    let starIds: string[] = this.listStarsOwnedByPlayer(
      game.galaxy.stars,
      playerId
    ).map((s) => s._id.toString());

    if (game.settings.diplomacy.enabled === "enabled") {
      // This never occurs when alliances is disabled.
      starIds = starIds.concat(
        this.listStarIdsWithPlayerCarriersInOrbit(game, playerId)
      );
    }
  ...
  isStarWithinScanningRangeOfStars(game: Game, star: Star, stars: Star[]) {
    ...
    // if KingOfHill, make the kingOfHill star always visible
    if (this.gameTypeService.isKingOfTheHillMode(game)) {
      if (star.isKingOfTheHillStar) {
        return true;
      }
    }
    ...
    for (let otherStar of stars) {
      ...
      let distance = this.starDistanceService.getDistanceBetweenStars(
        star,
        otherStar
      );
  ...
  filterStarsByScanningRange(game: Game, playerIds: DBObjectId[]) {
    ...
    let starsOwnedOrInOrbit = this.listStarsOwnedOrInOrbitByPlayers(
      game,
      playerIds
    );
    let starsWithScanning = starsOwnedOrInOrbit.filter(
      (s) => !this.isDeadStar(s)
    );

    // Seed the stars that are in range to be the stars owned or are in orbit of.
    let starsInRange: MapObjectWithVisibility[] = starsOwnedOrInOrbit.map(
      (s) => {
        return {
          _id: s._id,
          location: s.location,
    ...
    let starsToCheck: MapObjectWithVisibility[] = game.galaxy.stars
      .filter(
        (s) =>
          starsInRange.find((r) => r._id.toString() === s._id.toString()) ==
          null
      )
      .map((s) => {
        return {
    ...
    for (let star of starsWithScanning) {
      let starIds = this.getStarsWithinScanningRangeOfStarByStarIds(
        game,
        star,
        starsToCheck
      );
      ...
      for (let starId of starIds) {
        if (
          starsInRange.find(
            (x) => x._id.toString() === starId._id.toString()
          ) == null
        ) {
    ...
    for (let wormHoleStar of wormHoleStars) {
      if (
        starsInRange.find(
          (s) => s._id.toString() === wormHoleStar.destination._id.toString()
        ) == null
      ) {
        starsInRange.push({
  ...
  filterStarsByScanningRangeAndWaypointDestinations(
    game: Game,
    playerIds: DBObjectId[]
  ) {
  ...
  getStarsWithinScanningRangeOfStarByStarIds(
    game: Game,
    star: Star,
    stars: MapObjectWithVisibility[]
  ) {
    ...
    let effectiveTechs =
      this.technologyService.getStarEffectiveTechnologyLevels(game, star, true);
    let scanningRangeDistance = this.distanceService.getScanningDistance(
      game,
      effectiveTechs.scanning
    );
    ...
    let starsInRange = stars.filter((s) => {
      return (
        s.isAlwaysVisible ||
        this.starDistanceService.getDistanceBetweenStars(s, star) <=
          scanningRangeDistance
      );
    });
          ...
          if (specialist && specialist.modifiers.special) {
            if (specialist.modifiers.special.addNaturalResourcesOnTick) {
              this.addNaturalResources(
                game,
                star,
                specialist.modifiers.special.addNaturalResourcesOnTick
              );
            }
          }
        }
  ...
  captureStar(
    ...
    // Capture the star.
    let newStarPlayer = attackers.find(
      (p) => p._id.toString() === capturePlayerId.toString()
    )!;
    let newStarUser = attackerUsers.find(
      (u) =>
        newStarPlayer.userId &&
        u._id.toString() === newStarPlayer.userId.toString()
    );
    let newStarPlayerCarriers = attackerCarriers.filter(
      (c) => c.ownedByPlayerId!.toString() === newStarPlayer._id.toString()
    );
    ...
    if (game.settings.specialGalaxy.starCaptureReward === "enabled") {
      captureReward =
        ...
        game.constants.star.captureRewardMultiplier; // Attacker gets X credits for every eco destroyed.

      // Check to see whether to double the capture reward.
      let captureRewardMultiplier =
    ...
    const oldStarUser =
      defenderUsers.find(
        (u) => owner.userId && u._id.toString() === owner.userId.toString()
      ) || null;
  ...
  pairWormHoleConstructors(game: Game) {
    const constructors = game.galaxy.stars.filter(
      (s) =>
        s.specialistId &&
        this.specialistService.getByIdStar(s.specialistId)?.modifiers.special
          ?.wormHoleConstructor
    );
    ...
    while (pairs--) {
      const starA =
        constructors[this.randomService.getRandomNumber(constructors.length)];
      const starB =
        constructors[this.randomService.getRandomNumber(constructors.length)];

      if (
        starA._id.toString() === starB._id.toString() ||
