...
export default class GameCreateService {
  ...
  async create(settings: GameSettings) {
    ...
    const isNewPlayerGame =
      settings.general.type === "new_player_rt" ||
      settings.general.type === "new_player_tb";
    ...
    if (settings.general.createdByUserId && !isTutorial) {
      ...
      let openGames = await this.gameListService.listOpenGamesCreatedByUser(
        settings.general.createdByUserId
      );
      let userIsGameMaster = await this.userService.getUserIsGameMaster(
        settings.general.createdByUserId
      );

      if (openGames.length && !userIsGameMaster) {
        throw new ValidationError(
          "Cannot create game, you already have another game waiting for players."
        );
      }

      if (userIsGameMaster && openGames.length > 5) {
        throw new ValidationError(
          "Game Masters are limited to 5 games waiting for players."
        );
      }
      ...
      let isEstablishedPlayer = await this.userService.isEstablishedPlayer(
        settings.general.createdByUserId
      );
    ...
    if (game.settings.general.name.indexOf(RANDOM_NAME_STRING) > -1) {
      ...
      game.settings.general.name = game.settings.general.name.replace(
        RANDOM_NAME_STRING,
        randomGameName
      );
    }
    ...
    if (canApplyBans) {
      ...
      if (game.settings.specialGalaxy.specialistCost !== "none") {
        ...
        game.settings.specialGalaxy.specialistBans = {
          star: [
            ...new Set(
              game.settings.specialGalaxy.specialistBans.star.concat(starBans)
            ),
          ],
          carrier: [
            ...new Set(
    ...
    console.log(settings.general.teamGame);
    let gameObject = await game.save();
    console.log(gameObject.settings.general.teamGame);
  ...
  _setGalaxyCenter(game: Game) {
    ...
    game.constants.distances.galaxyCenterLocation =
      this.mapService.getGalaxyCenter(starLocations);
  }
