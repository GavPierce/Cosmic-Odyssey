...
export default class GameTickService extends EventEmitter {
  ...
  async tick(gameId: DBObjectId) {
    let game = (await this.gameService.getByIdAll(gameId).catch((error) => {
      console.error("Error getting this game during tick job: ", error);
    }))!;
    ...
    console.log(
      ...
      }] - Game tick started at ${new Date().toISOString()}`
    );
  ...
  canTick(game: Game) {
    ...
    if (this.gameTypeService.isRealTimeGame(game)) {
      ...
      nextTick = moment(lastTick)
        .utc()
        .add(game.settings.gameTime.speed, "seconds");
      ...
      nextTick = moment(lastTick)
        .utc()
        .add(game.settings.gameTime.maxTurnWait, "minutes");
    } else {
  ...
  async _combatCarriers(game: Game, gameUsers: User[]) {
    ...
    let carrierPositions: CarrierPosition[] = game.galaxy.carriers
      ...
      .map((c) => {
        ...
        let locationNext =
          this.carrierMovementService.getNextLocationToWaypoint(game, c);
    ...
    for (let carrierPath in graph) {
      ...
      for (let i = 0; i < positions.length; i++) {
        ...
        let collisionCarriers: CarrierPosition[] = positions.filter(
          (c: CarrierPosition) => {
            return (
              c.carrier.ships! > 0 &&
              !c.carrier.isGift && // Is still alive and not a gift
              // Head to head combat:
              ((c.destination.toString() ===
                friendlyCarrier.source.toString() &&
                c.distanceToSourceCurrent <=
                  friendlyCarrier.distanceToDestinationCurrent &&
                c.distanceToSourceNext >=
                  friendlyCarrier.distanceToDestinationNext) ||
                // Combat from behind:
                (c.destination.toString() ===
                  friendlyCarrier.destination.toString() &&
                  c.distanceToDestinationCurrent <=
                    friendlyCarrier.distanceToDestinationCurrent &&
                  c.distanceToDestinationNext >=
                    friendlyCarrier.distanceToDestinationNext))
            );
          }
        );
        ...
        collisionCarriers =
          this._filterAvoidCarrierToCarrierCombatCarriers(collisionCarriers);
        ...
        let friendlyCarriers = collisionCarriers.filter(
          (c) =>
            c.carrier.ships! > 0 &&
            c.carrier.ownedByPlayerId!.toString() ===
              friendlyCarrier.carrier.ownedByPlayerId.toString()
        );
        ...
        let friendlyPlayer = this.playerService.getById(
          game,
          friendlyCarrier.carrier.ownedByPlayerId
        )!;
        ...
        if (isAlliancesEnabled) {
          const playerIds: DBObjectId[] = [
            ...new Set(combatCarriers.map((x) => x.ownedByPlayerId!)),
          ];

          const isAllPlayersAllied =
        ...
        await this.combatService.performCombat(
          game,
          gameUsers,
          friendlyPlayer,
          null,
  ...
  async _moveCarriers(game: Game, gameUsers: User[]) {
    ...
    for (let i = 0; i < carriersInTransit.length; i++) {
      ...
      let carrierMovementReport = await this.carrierMovementService.moveCarrier(
        game,
        gameUsers,
        carrierInTransit
      );
    ...
    for (let i = 0; i < combatStars.length; i++) {
      ...
      let carriersAtStar = game.galaxy.carriers.filter(
        (c) => c.orbiting && c.orbiting.toString() === combatStar._id.toString()
      );

      let starOwningPlayer = this.playerService.getById(
        game,
        combatStar.ownedByPlayerId!
      )!;
  ...
  async _combatContestedStars(game: Game, gameUsers: User[]) {
    ...
    for (let i = 0; i < contestedStars.length; i++) {
      ...
      let starOwningPlayer = this.playerService.getById(
        game,
        contestedStar.star.ownedByPlayerId!
      )!;
  ...
  async _captureAbandonedStars(game: Game, gameUsers: User[]) {
    ...
    for (let i = 0; i < contestedAbandonedStars.length; i++) {
      ...
      let carrier = contestedStar.carriersInOrbit.sort(
  ...
  async _endOfGalacticCycleCheck(game: Game) {
    ...
    if (hasProductionTicked) {
      ...
      for (let i = 0; i < game.galaxy.players.length; i++) {
        ...
        let carrierUpkeepResult = this.playerService.deductCarrierUpkeepCost(
          game,
          player
        );
        ...
        if (this.diplomacyUpkeepService.isAllianceUpkeepEnabled(game)) {
          ...
          allianceUpkeepResult = this.diplomacyUpkeepService.deductTotalUpkeep(
            game,
            player,
    ...
    for (let i = 0; i < undefeatedPlayers.length; i++) {
      ...
      if (player.defeated) {
        ...
        let user = gameUsers.find(
          (u) => player.userId && u._id.toString() === player.userId.toString()
        );
  ...
  async _playAI(game: Game) {
    for (let player of game.galaxy.players.filter((p) =>
      this.playerAfkService.isAIControlled(game, p, true)
    )) {
