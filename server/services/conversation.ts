...
function getNewConversation(
  ...
  if (existingConvo) {
    throw new ValidationError(
      `A conversation already exists with the selected participants named [${existingConvo.name}].`
    );
  }
...
export default class ConversationService extends EventEmitter {
  ...
  createConversationAllPlayers(game: Game): void {
    ...
    let newMessage: ConversationMessage = {
      fromPlayerId: null,
      fromPlayerAlias: "Cosmic Odyssey",
      message:
        "Welcome to " +
        name +
  ...
  async list(game: Game, playerId: DBObjectId) {
    ...
    return convos.map((c) => {
      ...
      const unreadCount = msgs.filter(
        (m) =>
          m.readBy.find((r) => r.toString() === playerId.toString()) == null
      ).length;
      const isMuted =
        c.mutedBy!.find((m) => m.toString() === playerId.toString()) != null;
  ...
  async privateChatSummary(
    ...
    let convo = convos
      ...
      .find((c) => {
        return (
          c.participants.find((a) => a.toString() === playerIdA.toString()) &&
  ...
  _getConversationForPlayer(
    ...
    if (
      convo.participants.find(
        (p) => (p.toString() === playerId.toString()) == null
      )
    ) {
      throw new ValidationError(
  ...
  async detail(game: Game, playerId: DBObjectId, conversationId: DBObjectId) {
    ...
    convo.isMuted =
      convo.mutedBy!.find((m) => m.toString() === playerId.toString()) != null;
    ...
    if (convo.participants.length === 2) {
      ...
      let tradeEvents = await this.tradeService.listTradeEventsBetweenPlayers(
        game,
        playerId,
        convo.participants
      );
    ...
    convo.messages = convo.messages.sort(
      (a, b) => moment(a.sentDate).valueOf() - moment(b.sentDate).valueOf()
    );
  ...
  async send(
    ...
    conversationId: DBObjectId,
    message: string
  ): Promise<ConversationMessageSentResult> {
    ...
    if (
      convo.participants.find(
        (p) => (p.toString() === player._id.toString()) == null
      )
    ) {
      throw new ValidationError(
        `You are not participating in this conversation.`
    ...
    if (
      !newMessage.readBy.find((r) => r.toString() === player._id.toString())
    ) {
  ...
  getUnreadCount(game: Game, playerId: DBObjectId) {
    return (game.conversations || [])
      ...
      .reduce((sum, c) => {
        return (
          sum +
          (c.messages as ConversationMessage[]).filter(
            (m) =>
              m.readBy.find((r) => r.toString() === playerId.toString()) == null
          ).length
        );
