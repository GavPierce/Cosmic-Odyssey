...
export default class NotificationService {
  ...
  initialize() {
    if (this.discordService.isConnected()) {
      // Don't initialize the notification service if there's no token configured.
      this.conversationService.on(
        ConversationServiceEvents.onConversationMessageSent,
        (args) => this.onConversationMessageSent(args)
      );

      this.gameJoinService.on(GameJoinServiceEvents.onGameStarted, (args) =>
        this.onGameStarted(args)
      );
      this.gameTickService.on(GameTickServiceEvents.onGameEnded, (args) =>
        this.onGameEnded(args)
      );
      this.gameTickService.on(GameTickServiceEvents.onGameTurnEnded, (args) =>
        this.onGameTurnEnded(args)
      );
      this.gameTickService.on(
        GameTickServiceEvents.onPlayerGalacticCycleCompleted,
        (args) => this.onPlayerGalacticCycleCompleted(args)
      );
      this.researchService.on(
        ResearchServiceEvents.onPlayerResearchCompleted,
        (args) =>
          this.onPlayerResearchCompleted(
            args.gameId,
            args.playerId,
            args.technologyKey,
            args.technologyLevel,
            args.technologyKeyNext,
            args.technologyLevelNext
          )
      );
      this.tradeService.on(TradeServiceEvents.onPlayerCreditsReceived, (args) =>
        this.onPlayerCreditsReceived(
          args.gameId,
          args.fromPlayer,
          args.toPlayer,
          args.amount
      ...
      this.tradeService.on(
        TradeServiceEvents.onPlayerCreditsSpecialistsReceived,
        (args) =>
          this.onPlayerCreditsSpecialistsReceived(
            args.gameId,
            args.fromPlayer,
            args.toPlayer,
            args.amount
      ...
      this.tradeService.on(TradeServiceEvents.onPlayerRenownReceived, (args) =>
        this.onPlayerRenownReceived(
          args.gameId,
          args.fromPlayer,
          args.toPlayer,
          args.amount
      ...
      this.tradeService.on(
        TradeServiceEvents.onPlayerTechnologyReceived,
        (args) =>
          this.onPlayerTechnologyReceived(
            args.gameId,
            args.fromPlayer,
            args.toPlayer,
            args.technology
  ...
  async onPlayerGalacticCycleCompleted(
    ...
    await this._trySendNotifications(
      ...
      async (game: Game, user: User) => {
        ...
        if (
          args.carrierUpkeep &&
          (args.carrierUpkeep.carrierCount || args.carrierUpkeep.totalCost)
        ) {
          template.fields.push({
            name: "Carrier Upkeep",
        ...
        if (
          args.allianceUpkeep &&
          (args.allianceUpkeep.allianceCount || args.allianceUpkeep.totalCost)
        ) {
          template.fields.push({
