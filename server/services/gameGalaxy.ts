...
export default class GameGalaxyService {
  ...
  _setStarInfoBasic(doc: Game) {
    ...
    doc.galaxy.stars = doc.galaxy.stars.map((s) => {
      ...
      star.effectiveTechs =
        this.technologyService.getStarEffectiveTechnologyLevels(doc, star);
  ...
  _setStarInfoDetailed(
    ...
    perspectivePlayerIds: DBObjectId[]
  ) {
    ...
    if (!isFinished && (isDarkMode || (isDarkStart && !doc.state.startDate))) {
      if (isDarkMode) {
        doc.galaxy.stars =
          this.starService.filterStarsByScanningRangeAndWaypointDestinations(
            doc,
            perspectivePlayerIds
          );
      } else {
        doc.galaxy.stars = this.starService.filterStarsByScanningRange(
          doc,
          perspectivePlayerIds
        );
      }
    ...
    if (perspectivePlayerIds?.length) {
      ...
      playerScanningStars =
        this.starService.listStarsWithScanningRangeByPlayers(
          doc,
          perspectivePlayerIds
        );
      playerCarriersInOrbit =
        this.carrierService.listCarriersOwnedByPlayersInOrbit(
          doc.galaxy.carriers,
          perspectivePlayerIds
        );
    }
    ...
    doc.galaxy.stars = (doc.galaxy.stars as any[]) // TODO: Doing this to get around the whacky TS errors when deleting fields from the model
      .map((s) => {
        ...
        s.effectiveTechs =
          this.technologyService.getStarEffectiveTechnologyLevels(doc, s);
        ...
        let isOwnedByCurrentPlayer = playerStars.find(
          (y) => y._id.toString() === s._id.toString()
        );
        ...
        s.isInScanningRange =
          isFinished || // The game is finished
          this.starService.isStarWithinScanningRangeOfStars(
            doc,
            s,
            playerScanningStars
          ) || // The star is within scanning range
          playerCarriersInOrbit.find(
            (c) => c.orbiting!.toString() === s._id.toString()
          ) != null; // The star has a friendly carrier in orbit
        ...
        if (s.isInScanningRange) {
          ...
          let canSeeStarShips =
            perspectivePlayerIds?.length &&
            this.starService.canPlayersSeeStarShips(s, perspectivePlayerIds);
  ...
  _setCarrierInfoDetailed(doc: Game, perspectivePlayerIds: DBObjectId[]) {
    ...
    if (!this.gameStateService.isFinished(doc)) {
      doc.galaxy.carriers = this.carrierService.filterCarriersByScanningRange(
        doc,
        perspectivePlayerIds
      );
    ...
    doc.galaxy.carriers.forEach((c) => {
      c.effectiveTechs =
        this.technologyService.getCarrierEffectiveTechnologyLevels(doc, c);
  ...
  async _setPlayerInfoBasic(
    doc: Game,
    userPlayer: Player | null,
    perspectivePlayerIds: DBObjectId[] | null
  ) {
    ...
    if (!this.gameTypeService.isAnonymousGame(doc)) {
      let userIds: DBObjectId[] = doc.galaxy.players
        .filter((x) => x.userId)
        .map((x) => x.userId!);
    ...
    if (userPlayer) {
      playersInRange = this.playerService.getPlayersWithinScanningRangeOfPlayer(
        doc,
        doc.galaxy.players,
        userPlayer
      );
    ...
    doc.galaxy.players = doc.galaxy.players.map((p) => {
      ...
      p.hasPerspective =
        perspectivePlayerIds?.find((i) => i.toString() === p._id.toString()) !=
        null;
      ...
      p.isAIControlled = this.playerAfkService.isAIControlled(
        doc,
        p,
        !isCurrentUserPlayer
      );
      ...
      p.isInScanningRange =
        playersInRange.find((x) => x._id.toString() === p._id.toString()) !=
        null;
      p.shape = p.shape || "circle"; // TODO: I don't know why the shape isn't being returned by mongoose defaults.
      p.avatar = p.avatar
        ? avatars.find((a) => a.id.toString() === p.avatar!.toString())!.file
      ...
      if (!displayOnlineStatus) {
        ...
        p.isOnline =
          isCurrentUserPlayer ||
          onlinePlayers.find((op) => op._id.toString() === p._id.toString()) !=
            null;
      }
  ...
  _getPlayerPerspectives(game: Game, userId: DBObjectId | null) {
    ...
    if (userId && this.spectatorService.isSpectatingEnabled(game)) {
      let spectating = this.spectatorService.listSpectatingPlayers(
        game,
        userId
      );
  ...
  async _maskGalaxy(
    ...
    if (
      ...
      !history.stars.length ||
      !history.carriers.length
    ) {
      return;
    }
    ...
    for (let i = 0; i < game.galaxy.stars.length; i++) {
      ...
      if (
        ...
        gameStar.ownedByPlayerId.toString() === userPlayer._id.toString()
      ) {
        continue;
      }

      let historyStar = history.stars.find(
      ...
      if (historyStar) {
        ...
        if (
          ...
          historyStar.ownedByPlayerId.toString() === userPlayer._id.toString()
        ) {
          continue;
        }
    ...
    for (let i = 0; i < game.galaxy.carriers.length; i++) {
      ...
      if (
        !isHistorical &&
        userPlayer &&
        gameCarrier.ownedByPlayerId!.toString() === userPlayer._id.toString()
      ) {
        continue;
      }
    ...
    if (isHistorical) {
      for (let historyCarrier of history.carriers) {
        let gameCarrier = game.galaxy.carriers.find(
          (x) => x._id.toString() === historyCarrier.carrierId.toString()
        );
    ...
    if (!isHistorical) {
      const carrierInOrbitToAdd = history.carriers.filter(
        (c) =>
          c.orbiting &&
          (!userPlayer ||
            c.ownedByPlayerId!.toString() !== userPlayer._id.toString())
      );
