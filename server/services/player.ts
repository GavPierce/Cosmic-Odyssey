...
export default class PlayerService extends EventEmitter {
  ...
  getPlayersWithinScanningRangeOfPlayer(
    game: Game,
    players: Player[],
    player: Player
  ) {
    let inRange = [player];
    let playerStars = this.starService.listStarsWithScanningRangeByPlayer(
      game,
      player._id
    );
  ...
  isInScanningRangeOfPlayer(
    ...
    return (
      this.getPlayersWithinScanningRangeOfPlayer(
        game,
        [targetPlayer],
        sourcePlayer
  ...
  _generatePlayerColourShapeList(playerCount: number) {
    ...
    while (result.length !== playerCount) {
      ...
      let shapeColourIndex = this.randomService.getRandomNumber(
      ...
      let existingColour = result.find(
        (r) => r.colour.alias === shapeColour.colour.alias
      );
  ...
  _getDesiredPlayerDistanceFromCenter(game: Game) {
    ...
    if (game.settings.galaxy.galaxyType === "doughnut") {
      distanceFromCenter =
        (this.starDistanceService.getMaxGalaxyDiameter(locations) / 2) *
        (3 / 4);
    } else if (game.settings.galaxy.galaxyType === "spiral") {
      distanceFromCenter =
        this.starDistanceService.getMaxGalaxyDiameter(locations) / 2 / 2;
    } else {
  ...
  _getNewPlayerHomeStarCircular(
    ...
    let homeStar = this.starDistanceService.getClosestUnownedStarFromLocation(
      startingLocation,
      game.galaxy.stars
    );
  ...
  ownsOriginalHomeStar(game: Game, player: Player) {
    ...
    return (
      stars.find((s) => s._id.toString() === player.homeStarId!.toString()) !=
      null
    );
  }
