...
export default class CombatService extends EventEmitter {
  ...
  calculateStar(
    ...
    defenders: Player[],
    attackers: Player[],
    defenderCarriers: Carrier[],
    attackerCarriers: Carrier[],
    calculateNeeded: boolean = false
  ) {
    let combatWeapons = this._calculateEffectiveWeaponsLevels(
      game,
      star,
      defenders,
      attackers,
  ...
  _calculateEffectiveWeaponsLevels(
    game: Game,
    star: Star | null,
    defenders: Player[],
    attackers: Player[],
    ...
    defenderWeaponsTechLevel = Math.max(
      defenderWeaponsTechLevel - defenderWeaponsDeduction,
      1
    );
    attackerWeaponsTechLevel = Math.max(
      attackerWeaponsTechLevel - attackerWeaponsDeduction,
      1
    );
    ...
    let shouldSwapWeaponsTech =
      (isCarrierToStarCombat && attackerSwapWeapons && !defenderSwapWeapons) || // Attacker controls controls the weapon swap in c2s combat unless both have jokers
  ...
  async performCombat(
    ...
    let defenderCarriers = carriers
      .filter(
        (c) =>
          (c.ships! > 0 &&
            !c.isGift &&
            c.ownedByPlayerId!.toString() === defender._id.toString()) ||
          defenderAllies.find(
            (a) => a._id.toString() === c.ownedByPlayerId!.toString()
          ) // Either owned by the defender or owned by an ally
      )
    ...
    let defenderPlayerIds = defenderCarriers.map((c) =>
      c.ownedByPlayerId!.toString()
    );
    defenderPlayerIds.push(defender._id.toString());
    defenderPlayerIds = [...new Set(defenderPlayerIds)];

    let attackerPlayerIds = [
      ...new Set(attackerCarriers.map((c) => c.ownedByPlayerId!.toString())),
    ];

    let defenders: Player[] = defenderPlayerIds.map(
      (playerId) => this.playerService.getById(game, playerId as any)!
    );
    let attackers: Player[] = attackerPlayerIds.map(
      (playerId) => this.playerService.getById(game, playerId as any)!
    );
    ...
    for (let defender of defenders) {
      let user = gameUsers.find(
        (u) =>
          defender.userId && u._id.toString() === defender.userId.toString()
      );
    ...
    for (let attacker of attackers) {
      let user = gameUsers.find(
        (u) =>
          attacker.userId && u._id.toString() === attacker.userId.toString()
      );
    ...
    if (star) {
      combatResultShips = this.calculateStar(
        game,
        star,
        defenders,
        attackers,
      ...
      combatResultShips = this.calculateCarrier(
        game,
        defenders,
        attackers,
        defenderCarriers,
    ...
    combatResult.carriers = carriers.map((c) => {
      let specialist = this.specialistService.getByIdCarrierTrim(
        c.specialistId
      );
      let scrambled =
        (star &&
          star.isNebula &&
          defenderPlayerIds.includes(c.ownedByPlayerId!.toString())) ||
        this.specialistService.getCarrierHideShips(c);
    ...
    if (star) {
      ...
      let scrambled =
        star.isNebula || this.specialistService.getStarHideShips(star);
    ...
    this._distributeDamage(
      combatResult,
      attackerCarriers,
      combatResult.lost.attacker,
      true
    );
    this._distributeDamage(
      combatResult,
      defenderObjects,
      combatResult.lost.defender,
      true
    );

    if (!this.gameTypeService.isTutorialGame(game)) {
      this._updatePlayersCombatAchievements(
        combatResult,
        defenders,
        defenderUsers,
        defenderCarriers,
    ...
    if (attackerPlayerIds.length > 1) {
      ...
      if (star) {
        ...
        defender = this.playerService.getById(
          game,
          attackerPlayerIds[0] as any
        )!;
      }
  ...
  _starDefeatedCheck(
    ...
    let starDefenderDefeated =
      star && !Math.floor(star.shipsActual!) && !defenderCarriers.length;
    let hasAttackersRemaining =
      attackerCarriers.reduce((sum, c) => sum + c.ships!, 0) > 0;
    let hasCapturedStar = starDefenderDefeated && hasAttackersRemaining;

    if (hasCapturedStar) {
      return this.starService.captureStar(
        game,
        star,
        owner,
        defenders,
  ...
  _distributeDamage(
    ...
    while (shipsToKill) {
      ...
      let shipsPerObject = Math.max(
        1,
        Math.floor(shipsToKill / objectsToDeduct.length)
      );

      for (let obj of objectsToDeduct) {
        let isCarrier = obj.shipsActual == null;
        let combatObject: CombatCarrier | CombatStar =
          (combatResult.carriers.find(
  ...
  _updatePlayersCombatAchievements(
    combatResult: CombatResult,
    defenders: Player[],
    defenderUsers: User[],
    defenderCarriers: Carrier[],
    ...
    let defenderCarriersDestroyed = defenderCarriers.filter(
      (c) => !c.ships
    ).length;
    let defenderSpecialistsDestroyed = defenderCarriers.filter(
      (c) => !c.ships && c.specialistId
    ).length;

    let attackerCarriersDestroyed = attackerCarriers.filter(
      (c) => !c.ships
    ).length;
    let attackerSpecialistsDestroyed = attackerCarriers.filter(
      (c) => !c.ships && c.specialistId
    ).length;

    // Add combat result stats to defender achievements.
    for (let defenderUser of defenderUsers) {
      let defender = defenders.find(
        (u) => u.userId && u.userId.toString() === defenderUser._id.toString()
      )!;

      if (defender && !defender.defeated) {
        let playerCarriers = defenderCarriers.filter(
          (c) => c.ownedByPlayerId!.toString() === defender._id.toString()
        );

        defenderUser.achievements.combat.kills.ships +=
          combatResult.lost.attacker;
        defenderUser.achievements.combat.kills.carriers +=
          attackerCarriersDestroyed;
        defenderUser.achievements.combat.kills.specialists +=
          attackerSpecialistsDestroyed;

        defenderUser.achievements.combat.losses.ships +=
          combatResult.lost.defender; // TODO: This will not be correct in combat where its more than 2 players.
        defenderUser.achievements.combat.losses.carriers +=
          playerCarriers.filter((c) => !c.ships).length;
        defenderUser.achievements.combat.losses.specialists +=
          playerCarriers.filter((c) => !c.ships && c.specialistId).length;
      }
    }

    // Add combat result stats to attacker achievements.
    for (let attackerUser of attackerUsers) {
      let attacker = attackers.find(
        (u) => u.userId && u.userId.toString() === attackerUser._id.toString()
      )!;

      if (attacker && !attacker.defeated) {
        let playerCarriers = attackerCarriers.filter(
          (c) => c.ownedByPlayerId!.toString() === attacker._id.toString()
        );

        attackerUser.achievements.combat.kills.ships +=
          combatResult.lost.defender;
        attackerUser.achievements.combat.kills.carriers +=
          defenderCarriersDestroyed;
        attackerUser.achievements.combat.kills.specialists +=
          defenderSpecialistsDestroyed;

        attackerUser.achievements.combat.losses.ships +=
          combatResult.lost.attacker; // TODO: This will not be correct in combat where its more than 2 players.
        attackerUser.achievements.combat.losses.carriers +=
          playerCarriers.filter((c) => !c.ships).length;
        attackerUser.achievements.combat.losses.specialists +=
          playerCarriers.filter((c) => !c.ships && c.specialistId).length;
      }
    }
  }
  ...
  _shouldSwapWeaponsTech(carriers: Carrier[]) {
    return (
      carriers
        ...
        .find(
          (c) =>
            this.specialistService.getByIdCarrier(c.specialistId)?.modifiers
              .special?.combatSwapWeaponsTechnology
        ) != null
    );
  }
  ...
  sanitiseCombatResult(combatResult: CombatResult, player: Player) {
    ...
    if (result.star) {
      result.star = this.tryMaskObjectShips(
        combatResult.star,
        player
      ) as CombatStar;
    }

    result.carriers = combatResult.carriers.map((c) =>
      this.tryMaskObjectShips(c, player)
    ) as CombatCarrier[];
  ...
  tryMaskObjectShips(
    carrierOrStar: CombatStar | CombatCarrier | null,
    player: Player
  ) {
    if (!carrierOrStar) {
    ...
    if (
      carrierOrStar.scrambled &&
      carrierOrStar.ownedByPlayerId &&
      player._id.toString() !== carrierOrStar.ownedByPlayerId.toString()
    ) {
