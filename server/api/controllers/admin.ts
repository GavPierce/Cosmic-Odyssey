...
export default (container: DependencyContainer) => {
  return {
    ...
    listUsers: async (req, res, next) => {
      try {
        let result = await container.adminService.listUsers(
          req.session.roles.administrator,
          300
        );
    ...
    setCredits: async (req, res, next) => {
      try {
        await container.userService.setCredits(
          req.params.userId,
          req.body.credits
        );
    ...
    impersonate: async (req, res, next) => {
      try {
        ...
        await container.emailService.send(
          "gavinpierce97@gmail.com",
          "Impersonation Attempt",
          `User ${req.session.username} is attempting to impersonate ${user.username}"`
        );
